...
def admin_stats():
    ...
    with get_db() as conn:
        ...
        reviewing   = conn.execute("SELECT COUNT(*) FROM applications WHERE status='Reviewing'").fetchone()[0]
        ...
        # Applications over time (dummy grouped by recent months)
        # Assuming applied_at is just dates, we will mock trend data if there isn't enough, 
        # or properly group if there is.
        import datetime
